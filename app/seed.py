...
async def seed_demo_scenario() -> None:
    """Seed the demo «Перевод выше лимита» scenario for PER-43.

    Six steps that walk Login → Profile → Transfer → enter amount over
    the 100k limit → press the button. Step 6's expected_result quotes
    the spec's «Превышен лимит» wording so PER-37's auto-RAG-check
    flags the buggy «Неверная карта» response as spec_mismatch.

    Idempotent: skips if a scenario with the same title already exists
    in the workspace. Workspace defaults to the first one in the DB
    (single-tenant demo); on multi-tenant deployments the admin can
    ...
    SAMPLE_TITLE = "Перевод выше лимита (демо)"

    async with async_session_maker() as session:
        ws = (await session.execute(_select(Workspace).limit(1))).scalar_one_or_none()
        if ws is None:
            return
        existing = (
            await session.execute(
                _select(Scenario).where(
                    Scenario.workspace_id == ws.id,
                    Scenario.title == SAMPLE_TITLE,
                )
            )
        ).scalar_one_or_none()
        ...
        # Author = first admin user; fall back to first user in the DB.
        author = (
            await session.execute(
                _select(User).where(User.is_superuser.is_(True)).limit(1)
            )
        ).scalar_one_or_none() or (
            await session.execute(_select(User).limit(1))
        ).scalar_one_or_none()
        if author is None:
            return
        ...
        steps = [
            ...
            {"screen_name": "Login",   "action": "input",
             "element_label": "Пароль",
             "value": "{{test_data.password}}",
             "expected_result": "Символы скрыты"},
            {"screen_name": "Login",   "action": "tap",
             "element_label": "Войти",
             "expected_result": "Переход на экран профиля"},
            {"screen_name": "Profile", "action": "tap",
             "element_label": "Перевод",
             "expected_result": "Открылся экран Перевод"},
            {"screen_name": "Transfer", "action": "input",
             "element_label": "Сумма",
             "value": "200000",
             "expected_result": "Сумма 200000 введена в поле"},
            {"screen_name": "Transfer", "action": "tap",
             "element_label": "Перевести",
             "expected_result": (
                 "При превышении лимита 100 000 ₽ должно появиться "
                 "сообщение «Превышен лимит» и операция должна быть "
                 "заблокирована"
             )},
        ]
        session.add(Scenario(
            ...
            title=SAMPLE_TITLE,
            description=(
                "Демо для RAG-сверки: агент пытается перевести 200 000 ₽, "
                "приложение должно показать «Превышен лимит». "
                "Привяжите к сценарию документ demo/spec.md (раздел "
                "«Перевод») в настройках сценария — RAG найдёт расхождение."
            ),
            steps_json={"steps": steps},
            is_active=True,
            # rag_document_ids остаётся NULL — пользователь привяжет
            # вручную после загрузки spec.md в knowledge.
            rag_document_ids=None,
        ))
        await session.commit()
        print(f"[seed] Demo scenario '{SAMPLE_TITLE}' inserted")
