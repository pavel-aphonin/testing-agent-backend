...
def upgrade() -> None:
    ...
    # 3. Seed 11 unassigned rows. UUIDs generated by Postgres via
    # ``gen_random_uuid()`` — pgcrypto extension is already enabled
    # by earlier migrations. Doing the seed inline (not in
    # app/seed.py) keeps the schema migration self-contained: a
    # fresh DB has all 11 rows after a single ``alembic upgrade
    # head``, with no application-layer hook required.
    for role in _ROLES:
        op.execute(
            sa.text(
                "INSERT INTO module_assignments (id, role, llm_model_id) "
                "VALUES (gen_random_uuid(), :role, NULL)"
            ).bindparams(sa.bindparam("role", value=role))
        )
